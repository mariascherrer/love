...
window.onload = function () {
    ...
    player.move = function (direction) {
        ...
        if (player.x == 6 && player.y == 7 && score >= 5) {
    ...
    function handleTouchStart(event) {
        ...
        // Verifica se o toque ocorreu nos botões de movimento
        var touchX = event.touches[0].pageX;
        var touchY = event.touches[0].pageY;

        if (touchX < canvas.width / 2) {
            player.currentDirection = "right-1";// Movimento para a esquerda
        } else {
            player.x += player.speed; // Movimento para a direita
        }
    }
